...
namespace View {
	...
	public class Node : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler, IPositionChangeHandler {
		...
		public struct Metadata {
			...
			[SerializeField] public WindowStates WindowState;
		...
		public void RestoreFromMetadata ( Metadata metadata ) {

			transform.position = metadata.Position;
			_windowState = metadata.WindowState;
		}
		...
		// properties
		public Library.Model.Node Data {
			get => _data;
			set {
				...
				HandleTitleChange( new Library.Model.Node.TitleChangedEventData( null, _data.Title ) );
		...
		[SerializeField] private Library.Model.Node _data;
		[SerializeField] private Library.Model.Style _style;
		...
		private void SetEditState ( EditStates editState ) {
			...
			switch ( _editState ) {
				...
				case EditStates.View:
					...
					// reformat body
					var content = new Library.Model.Content( userString: _data.Body );
		...
		// data event handlers
		private void HandleTitleChange ( Library.Model.Node.TitleChangedEventData eventData ) {
			...
			switch ( _editState ) {
				...
				case EditStates.View:
					_titleInputField.text = eventData.NewTitle;
					break;
			}
		}
		private void HandleBodyChanged ( string body ) {
			...
			switch ( _editState ) {
				...
				case EditStates.View:
					var content = new Library.Model.Content( userString: body );
					_content.SetContentModel( content );
					_contentInputField.text = _content.TMPString;
					break;
			}

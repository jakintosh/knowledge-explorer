...
namespace View {
	...
	public struct Link {
		...
		public override string ToString ()
			=> $"ID: {ID}, Char Range: {{{StartIndex} -> {EndIndex}}}";
	}
	...
	public class Content {
		...
		private Library.Model.Content _content;
		private Library.Model.Style _style;

		public Content ( Library.Model.Content content, Library.Model.Style style ) {
		...
		public void SetStyle ( Library.Model.Style style ) {

			_style = style;
			Refresh();
		}
		public void SetContentModel ( Library.Model.Content content ) {
		...
		private string DoSomething ( string modelString ) {
			...
			for ( int i = 0; i < modelString.Length; i++ ) {
				...
				if ( c == '$' ) { // found link
					...
					var idLength = Library.Model.Bucket.NODE_ID_LENGTH;
					...
					// resolve title
					// var title = Library.Model.Bucket.Instance.GetTitleForID( id );
					var title = "View.cs/DoSomething";
					if ( title != null ) {

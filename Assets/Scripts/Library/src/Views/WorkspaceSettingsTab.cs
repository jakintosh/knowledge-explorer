...
namespace Library.Views {
	...
	public class WorkspaceSettingsTab : ReuseableView<string> {
		...
		protected override void OnInitialize () {
			...
			_workspaceUID = new Observable<string>(
				...
				onChange: uid => {
					...
					_workspaceName.Set( App.Workspaces.Get( uid )?.Name );
				}
			);
			...
			_deleteButton.onClick.AddListener( () => {
				Library.App.History.ExecuteAction(
					new Actions.Workspace.Delete( uid: _workspaceUID.Get(), shouldClose: true )
				);
				OnDismiss?.Invoke();
			} );
		...
		private void Save () {
			...
			Library.App.History.ExecuteAction(
				new Actions.Workspace.Rename(
					uid: _workspaceUID.Get(),
					name: _validatedName.Get().name
				)
			);
		}
